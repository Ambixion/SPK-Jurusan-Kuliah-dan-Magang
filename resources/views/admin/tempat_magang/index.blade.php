                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="skill-container" style="display:flex;flex-wrap:wrap;gap:6px;">
                                        @forelse($item->skills as $i => $skill)
                                            <span class="badge-custom badge-magang skill-item"
                                                style="margin:0;display:{{ $i < 3 ? 'inline-flex' : 'none' }};white-space:nowrap;"
                                                data-skill-index="{{ $i }}">{{ $skill->jenis_skill }}</span>
                                        @empty
                                            <span style="color:#4a5568;font-size:0.8rem;">Belum ada skill</span>
                                        @endforelse
                                        @if($item->skills->count() > 3)
                                            <button type="button" class="skill-toggle-btn"
                                                style="padding:4px 12px;font-size:0.75rem;background:#5b6af0;color:#fff;border:none;border-radius:4px;cursor:pointer;white-space:nowrap;"
                                                onclick="toggleSkills(this)">+{{ $item->skills->count() - 3 }} lainnya</button>
                                        @endif
                                    </div>
                                </td>
                                <td>{{ $item->bidang }}</td>
                                ...
                                <td>
                                    <div class="btn-group btn-group-sm" role="group">
                                        <a href="{{ route('admin.tempat_magang.edit', $item->id) }}" class="btn-sm-action btn-edit">✏
                                            Edit</a>
                                        <form method="POST" action="{{ route('admin.tempat_magang.destroy', $item->id) }}"
                                            style="d-inline" data-confirm-delete data-confirm-text="Yakin hapus user ini?">
                                            @csrf
                                            @method('DELETE')
